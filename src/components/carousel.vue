  ...
  <div class="carousel-wrapper">
    ...
    <div class="carousel" ref="carousel">
      <div class="carousel--overflow-container">
        <div class="carousel-items" :style="{ transform: 'translateX' + '(' + currentOffset + 'px' + ')'}" >
          <div ref="card" class="carousel--card" v-for="(item, index) in items" :key="index">
            ...
            <div v-else>
                ...
                <img v-if="item.profile_path === null || item.poster_path === null" id="no-photo" :src="noPhoto" alt='no profile found' />
                <img v-else-if="!item.profile_path" :src="`https://image.tmdb.org/t/p/w1280/${item.poster_path}`" :alt="`${item.original_title}`" />
                <img v-else :src="`https://image.tmdb.org/t/p/w1280/${item.profile_path}`" :alt="`${item.name}`" />
                <div v-if="!item.name" class="carousel--card--footer">
                    <p><strong>{{ item.original_title }}  ({{ item.release_date.substring(0,4) }})</strong></p>
                </div>
